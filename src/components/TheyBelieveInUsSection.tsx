...
const TheyBelieveInUsSection = () => {
  ...
  return (
    <section className="w-full h-[70vh] bg-white py-24 px-4 flex flex-col items-center">
      <h2 className="text-[4rem] font-normal text-[#111] mb-20 text-center leading-tight">They believe in us</h2>
      <div className="w-full max-w-8xl flex flex-col items-center">
        <div className="w-full border-t border-[#e5e7eb] mb-12" style={{height: 0, borderWidth: '1px', opacity: 0.7}} />
        <div className="w-full overflow-hidden relative">
          ...
          >
            {marqueeCompanies.map((company, idx) => (
              <span
                key={idx}
                className={`text-[2.2rem] ${company.bold ? 'font-bold' : 'font-normal'} text-[#111] flex items-center`}
                style={{ 
